...
test.describe('Login Page Tests', () => {
  ...
  test('should display all login form elements', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        const { page: videoPage, context: videoContext, cleanup } = await testService.createVideoContext('login-form-elements');
        ...
        try {
          ...
          await expect(videoTestService.getLocator(TEST_IDS.login.emailInput)).toBeVisible();

          // Verify password input
          await videoTestService.debugHighlight(TEST_IDS.login.passwordInput);
          await expect(videoTestService.getLocator(TEST_IDS.login.passwordInput)).toBeVisible();

          // Verify login button
          await videoTestService.debugHighlight(TEST_IDS.login.loginButton);
          await expect(videoTestService.getLocator(TEST_IDS.login.loginButton)).toBeVisible();

          // Verify forgot password link
          await videoTestService.debugHighlight(TEST_IDS.login.forgotPasswordLink);
          await expect(videoTestService.getLocator(TEST_IDS.login.forgotPasswordLink)).toBeVisible();

          // Verify signup link
          await expect(videoTestService.getLocator(TEST_IDS.login.signupLink)).toBeVisible();

          // Verify theme toggle
          await videoTestService.debugHighlight(TEST_IDS.login.themeToggle);
          await expect(videoTestService.getLocator(TEST_IDS.login.themeToggle)).toBeVisible();
        } finally {
          await cleanup();
        }
      },
  ...
  test('should successfully login with valid credentials', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        ...
        await testService.handleInput(TEST_IDS.login.emailInput, 'fill', validUser.email);
        await testService.handleInput(TEST_IDS.login.passwordInput, 'fill', validUser.password);
  ...
  test('should show error with invalid password', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        ...
        await testService.handleInput(TEST_IDS.login.emailInput, 'fill', validUser.email);
        await testService.handleInput(TEST_IDS.login.passwordInput, 'fill', invalidUser.password);
  ...
  test('should show error with invalid email', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        // Fill in invalid credentials
        await testService.handleInput(TEST_IDS.login.emailInput, 'fill', invalidUser.email);
        await testService.handleInput(TEST_IDS.login.passwordInput, 'fill', invalidUser.password);
  ...
  test('should validate form fields', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        ...
        expect(await testService.getLocator(`${TEST_IDS.login.emailInput}-error`).innerText()).toBe('Please enter your email');

        expect(await testService.getLocator(`${TEST_IDS.login.passwordInput}-error`).innerText()).toBe('Please enter your password');

        // Try invalid email format
        await testService.handleInput(TEST_IDS.login.emailInput, 'fill', 'invalid-email');
        await testService.handleInput(TEST_IDS.login.loginButton, 'click');
        expect(await testService.getLocator(`${TEST_IDS.login.emailInput}-error`).innerText()).toBe('Please enter a valid email address');
      },
      {
  ...
  test('should toggle between dark and light mode', async ({ page, context }) => {
    testService = new TestService({
      page,
      context,
      browserName: process.env.BROWSER || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        ...
        try {
          ...
          await videoTestService.handleInput(TEST_IDS.login.themeToggle, 'click');

          // Verify dark mode applied
          await videoTestService.waitForTimeout(500); // Wait for transition
          videoTestService.logger.info('Verifying dark mode styles');
          await videoTestService.highlightMultipleAndCapture([TEST_IDS.login.themeToggle, TEST_IDS.login.themeToggle]);
  ...
  test('should navigate to register form when clicking signup link', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
  ...
  test('should show validation error for forgot password email', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        ...
        await testService.handleInput(TEST_IDS.login.forgotPasswordLink, 'click');
  ...
  test('should display and allow clicking Google Sign-In button', async ({ page, context, browserName }) => {
    testService = new TestService({
      page,
      context,
      browserName: browserName || 'chromium',
    ...
    await testService.tryCatchHandler(
      async () => {
        const googleButton = testService.getLocator(TEST_IDS.login.googleSigninButton);
